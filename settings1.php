...
$f=0;
 $sql="SELECT name, salary FROM tb2 WHERE email='$mail';";
    $res=$conn->query($sql);
...
//changing user name
if($newname!=""){
    $sql="update tb2 set name='$newname' where email='$mail'";
...
//changing income
if($newincome!=""){
    $sql="update tb2 set salary='$newincome' where email='$mail'";
...
//changing password
  if($oldpassword!=""){ 
   ...
   if($f==0){
        $sql="update tb2 set password='$newpassword' where email='$mail'";
...
if($newname!="" || $newincome!="" || $newpassword!=""){
   ...
   if($newname=="") {$newname="#no_change";}
    $sql1="insert into changes1(email,oldname,newname,oldincome,newincome,oldpassword,newpassword,date,month,year) values('$mail','$oldname','$newname',$oldincome,$newincome,'$oldpassword','$newpassword',$date1,$month,$year)";
